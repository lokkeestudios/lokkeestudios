...
const {
  generateHelpUrl
} = require('@sanity/generate-help-url'); // NOTE: when doing changes here, also remember to update versions in help docs at
...
module.exports = workDir => {
  ...
  const installedPackages = PACKAGES.map(pkg => {
    ...
    const installed = semver.coerce(manifestPath ? readPackageJson(manifestPath).version : dependencies[pkg].replace(/[\D.]/g, ''));
    ...
    const isUntested = !semver.satisfies(installed, supported) && semver.gtr(installed, supported); // "Unsupported" in that the installed version is _lower than_ the minimum version
    // Ex: Installed is react@15.0.0, but we require react@^16

    const isUnsupported = !semver.satisfies(installed, supported) && !isUntested; // "Deprecated" in that we will stop supporting it at some point in the near future,
...
function getUpgradeInstructions(pkgs) {
  const inst = pkgs.map(pkg => {
    const [highestSupported] = pkg.supported.map(version => semver.coerce(version).version).sort(semver.rcompare);
