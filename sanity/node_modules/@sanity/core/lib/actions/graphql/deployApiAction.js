...
const {
  tryInitializePluginConfigs
} = require('../config/reinitializePluginConfigs');
...
module.exports = async function deployApiActions(args, context) {
  ...
  await tryInitializePluginConfigs({
    workDir,
    output,
    env: 'production'
  });
  ...
  if (!(await confirmValidationResult(valid, {
    spinner,
    output,
    prompt,
    force
  ...
  try {
    ...
    output.print(client.getUrl(response.location.replace(/^\/(v1|v\d{4}-\d{2}-\d{2})\//, '/'))); // Because of side effects when loading the schema, we can end up in situations where
...
function getCurrentSchemaProps({
  client,
  dataset,
  tag
}) {
...
function parseCliFlags(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).option('dataset', {
    type: 'string'
  }).option('tag', {
    type: 'string',
    default: 'default'
...
async function confirmValidationResult(valid, {
  spinner,
  output,
  prompt,
  force
...
async function resolveApiGeneration({
  ...
  if (!currentGeneration) {
    ...
    debug('There is no current generation deployed, using %s (%s)', generation, flags.generation ? 'specified' : 'default');
    return generation;
  }
  ...
  if (flags.generation && flags.generation !== currentGeneration) {
    ...
    output.warn(`Specified generation (${flags.generation}) differs from the one currently deployed (${currentGeneration}).`);
    const confirmDeploy = flags.force || (await prompt.single({
      type: 'confirm',
  ...
  if (generation !== latestGeneration) {
    output.warn(chalk.cyan(`A new generation of the GraphQL API is available, use \`--generation ${latestGeneration}\` to use it`));
  }
