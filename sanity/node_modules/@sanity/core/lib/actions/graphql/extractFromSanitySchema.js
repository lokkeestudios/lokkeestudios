...
function extractFromSanitySchema(sanitySchema, extractOptions = {}) {
  ...
  const unionTypes = [];
  const types = typeNames.map(name => sanitySchema.get(name)).filter(isBaseType).map(type => convertType(type)).concat(unionTypes);
  return {
    types,
  ...
  function getUnionDefinition(candidates, parent, options = {}) {
    ...
    try {
      ...
      const converted = candidates.map(def => convertType(def)); // We might end up with union types being returned - these needs to be flattened
      ...
      const inlineObjs = flattened.filter(type => !type.isReference).map(ref => ref.name); // Here we remove duplicates, as they might appear twice due to in-line usage of types as well as references

      const possibleTypes = [...new Set(flattened.map(type => type.isReference ? type.type : type.name))].sort();
