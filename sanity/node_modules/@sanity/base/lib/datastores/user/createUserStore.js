...
function fetchCurrentUser() {
  return (0, _rxjs.defer)(() => {
    ...
    if (!(0, _authState.authTokenIsAllowed)() || user || !user && !sid) {
      debug(user ? "Received user with ID ".concat(user.id) : 'Received no user'); // Nullify the session ID - in some cases (with multiple tabs/windows), we might
    ...
    debug('Session ID present in URL, but no user received - fetching token'); // Regardless of success or failure, we don't want to reuse the SID after this try

    var sessionId = sid;
    sid = null; // If we have consumed a session ID from the URL, we would expect a user to be returned,
    ...
    return (0, _authState.fetchToken)(sessionId, client).pipe((0, _operators.switchMap)(_ref => {
      ...
      debug('Token received - storing in localStorage'); // Save token to local storage
      ...
      _state.authStateChangedInThisWindow$.next(true); // Now try retrieving the user again
...
if ((_module = module) !== null && _module !== void 0 && (_module$hot = _module.hot) !== null && _module$hot !== void 0 && _module$hot.dispose) {
  module.hot.dispose(() => {
