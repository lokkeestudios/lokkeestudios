...
var results$ = operationCalls$.pipe((0, _operators.groupBy)(op => op.idPair.publishedId), (0, _operators.mergeMap)(groups$ => groups$.pipe( // although it might look like a but, dropping pending async operations here is actually a feature
// E.g. if the user types `publish` which is async and then starts patching (sync) then the publish
// should be cancelled
(0, _operators.switchMap)(args => (0, _operationArgs.operationArgs)(args.idPair, args.typeName).pipe((0, _operators.take)(1), (0, _operators.switchMap)(operationArguments => {
  ...
  var isConsistent$ = (0, _consistencyStatus.consistencyStatus)(args.idPair, args.typeName).pipe((0, _operators.filter)(Boolean));
...
var operationEvents = (0, _createMemoizer.memoize)(idPair => results$.pipe((0, _operators.filter)(result => result.args.idPair.publishedId === idPair.publishedId), (0, _operators.map)(result => {
  var _result$args = result.args,
      operationName = _result$args.operationName,
      documentIds = _result$args.idPair;
