...
function observePaths(value, paths, observeFields, apiConfig) {
  ...
  if (pathsWithMissingHeads.length > 0) {
    ...
    if (isRef || isDocument(value)) {
      ...
      var refApiConfig = // if it's a cross dataset reference we want to use it's `_projectId` + `_dataset`
      // attributes as api config
      isRef && value._dataset && value._projectId ? {
        projectId: value._projectId,
        dataset: value._dataset
      } : apiConfig;
      return observeFields(_id, nextHeads, refApiConfig).pipe((0, _operators.switchMap)(snapshot => {
        if (snapshot === null) {
          return (0, _rxjs.of)(null);
        }

        return observePaths(_objectSpread(_objectSpread(_objectSpread({}, createEmpty(nextHeads)), isRef ? _objectSpread({
          _ref: value._ref
        }, apiConfig) : value), snapshot), paths, observeFields, refApiConfig);
      }));
    }
...
function createPathObserver(observeFields) {
  return (value, paths, apiConfig) => observePaths(normalizeValue(value), normalizePaths(paths), observeFields, apiConfig);
}