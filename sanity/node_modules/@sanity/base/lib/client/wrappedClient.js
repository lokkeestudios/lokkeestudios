...
var _authState = require("../datastores/authState");
...
var wrappedClient = {
  config(newConfig) {
    ...
    if (!silence) {
      ...
      console.warn(new Error("Setting configuration on the global studio client is deprecated - please use `withConfig()` instead - see ".concat((0, _generateHelpUrl.generateHelpUrl)('studio-client-global-config'))));
  ...
  withConfig: newConfig => {
    if (!newConfig || !newConfig.apiVersion) {
      throw new Error("Client `withConfig()` called without an `apiVersion` - see ".concat((0, _generateHelpUrl.generateHelpUrl)('studio-client-specify-api-version')));
    }

    var newClient = configuredClient.clone().config(newConfig);
    instances.push(newClient); // Subscribe to auth token stream and configure the client depending on token value

    _authState.authToken$.subscribe(token => {
      var currentHasToken = Boolean(newClient.config().token);
      var nextHasToken = Boolean(token);

      if (currentHasToken !== nextHasToken) {
        newClient.config(_objectSpread(_objectSpread({}, newConfig), {}, {
          token
        }));
      }
    });
...
getKeys(configuredClient).forEach(key => {
  ...
  Object.defineProperty(wrappedClient, key, {
    get() {
      ...
      new Error("Used property \"".concat(key, "\" on versionless client - this is deprecated. Please specify API version using `withConfig` - see ").concat((0, _generateHelpUrl.generateHelpUrl)('studio-client-specify-api-version'))));
