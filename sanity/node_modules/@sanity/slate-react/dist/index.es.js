import React, { useRef, useLayoutEffect, useEffect, useState, Fragment, useContext, createContext, useMemo, useCallback } from 'react';
...
var TextString = props => {
  ...
  var ref = useRef(null); // This is the actual text rendering boundary where we interface with the DOM
  ...
  useLayoutEffect(() => {
    ...
    var textWithTrailing = "".concat(text !== null && text !== void 0 ? text : '').concat(isTrailing ? '\n' : '');
...
var Editable$1 = props => {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    try {
      newDomRange = selection ? ReactEditor.toDOMRange(editor, selection) : undefined;
    } catch (err) {// It's OK
      // console.error(
    ...
    if (newDomRange) {
      var startOffset = newDomRange.startOffset; // COMPAT: Firefox will not trig clipboard events when selecting void nodes.
      ...
      if (IS_FIREFOX && selection && startOffset === 1 && newDomRange.endOffset === 1 && newDomRange[Range.isBackward(selection) ? 'endContainer' : 'startContainer'].textContent === '\uFEFF') {
        var node = Node.parent(editor, selection.focus.path);
      ...
      if (Range.isBackward(selection)) {
        domSelection.setBaseAndExtent(newDomRange.endContainer, newDomRange.endOffset, newDomRange.startContainer, startOffset);
      } else {
        domSelection.setBaseAndExtent(newDomRange.startContainer, startOffset, newDomRange.endContainer, newDomRange.endOffset);
      }
  ...
  return /*#__PURE__*/React.createElement(ReadOnlyContext.Provider, {
    ...
    onKeyDown: useCallback(event => {
      if (!readOnly && !state.isComposing && hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onKeyDown)) {
        ...
        if (selection && (Hotkeys.isExtendUp(nativeEvent) || Hotkeys.isExtendDown(nativeEvent)) && editor.isVoid(element)) {
          event.preventDefault();
          var reverse = Hotkeys.isExtendUp(nativeEvent);
          Transforms.move(editor, {
            unit: 'line',
            edge: 'focus',
            reverse
        ...
        if ((Hotkeys.isMoveUp(nativeEvent) || Hotkeys.isMoveDown(nativeEvent)) && editor.isVoid(element)) {
          event.preventDefault();

          var _reverse = Hotkeys.isMoveDown(nativeEvent);
...
var ReactEditor = {
  ...
  toSlatePoint(editor, domPoint, options) {
    ...
    if (parentNode) {
      ...
      if (domNode && offset === domNode.textContent.length && ( // COMPAT: If the parent node is a Slate zero-width space, editor is
      ...
      IS_FIREFOX && (_domNode$textContent = domNode.textContent) !== null && _domNode$textContent !== void 0 && _domNode$textContent.endsWith('\n\n'))) {
        offset--;
      }
    }
