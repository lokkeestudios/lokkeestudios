...
function createWithPortableTextMarkModel(portableTextFeatures) {
  return function withPortableTextMarkModel(editor) {
    ...
    editor.normalizeNode = nodeEntry => {
      ...
      var isSpan = _slate.Text.isText(node) && node._type === portableTextFeatures.types.span.name;
      var isTextBlock = editor.isTextBlock(node);

      if (isSpan && _slate.Text.isText(node)) {
        if (!node.marks) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            if (op.type === 'merge_node' && op.path.length === 1 && _slate.Element.isElementProps(op.properties) && 'markDefs' in op.properties && op.properties._type === portableTextFeatures.types.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
            ...
            if (op.type === 'split_node' && op.path.length === 1 && _slate.Element.isElementProps(op.properties) && op.properties._type === portableTextFeatures.types.block.name && 'markDefs' in op.properties && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] + 1 < editor.children.length) {
              ...
              if (editor.isTextBlock(_targetBlock)) {
            ...
            if (op.type === 'split_node' && op.path.length === 2 && op.properties._type === portableTextFeatures.types.span.name && 'marks' in op.properties && Array.isArray(op.properties.marks) && op.properties.marks.length > 0 && op.path[0] + 1 < editor.children.length) {
              ...
              if (_slate.Text.isText(child) && child.text === '' && Array.isArray(child.marks) && child.marks.length > 0) {
                _slate.Transforms.setNodes(editor, {
                  marks: []
                }, {
                  at: childPath,
    ...
    editor.apply = op => {
      if (op.type === 'insert_text') {
        ...
        if (selection && _slate.Range.isCollapsed(selection) && (_Editor$marks = _slate.Editor.marks(editor)) !== null && _Editor$marks !== void 0 && (_Editor$marks$marks = _Editor$marks.marks) !== null && _Editor$marks$marks !== void 0 && _Editor$marks$marks.some(mark => !decorators.includes(mark))) {
          ...
          if (_slate.Text.isText(node) && node.text.length === selection.focus.offset && Array.isArray(node.marks) && node.marks.length > 0) {
            apply(op);
          ...
          _slate.Editor.withoutNormalizing(editor, () => {
            ...
            if (editor.selection) {
              ...
              splitTextNodes.forEach(_ref5 => {
                ...
                if (_slate.Element.isElement(block) && block.children.includes(node)) {
                  _slate.Transforms.setNodes(editor, {
      ...
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          if (_slate.Editor.isBlock(editor, parent)) {
            ...
            if (_slate.Text.isText(node) && _slate.Text.isText(nextNode) && (0, _isEqual2.default)(nextNode.marks, node.marks)) {
              debug('Merging spans');
          ...
          if (editor.isTextBlock(block)) {
            var newMarkDefs = block.markDefs.filter(def => {
              return block.children.find(child => {
                return _slate.Text.isText(child) && Array.isArray(child.marks) && child.marks.includes(def._key);
              });
            });
