...
function transformOperation(editor, patch, operation) {
  ...
  if (patch.type === 'diffMatchPatch') {
    ...
    if (_slate.Element.isElement(block) && Array.isArray(block.children)) {
      ...
      if ((operation.type === 'insert_text' || operation.type === 'remove_text') && _slate.Path.isPath(operation.path) && operation.path[0] !== undefined && operation.path[0] === blockIndex && operation.path[1] === childIndex) {
...
function adjustBlockPath(editor, patch, operation, level) {
  ...
  if (myIndex >= 0 && operation.type !== 'set_selection' && Array.isArray(operation.path) && operation.path[0] !== undefined && operation.path[0] >= myIndex + level) {
    var transformedOperation = _objectSpread({}, operation);
