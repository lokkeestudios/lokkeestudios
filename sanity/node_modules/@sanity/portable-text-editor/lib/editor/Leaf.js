...
var Leaf = props => {
  ...
  if (_slate.Text.isText(leaf) && leaf._type === portableTextFeatures.types.span.name) {
    var blockElement = children.props.parent;
    var path = blockElement ? [{
    ...
    var annotations = annotationMarks.map(mark => !decoratorValues.includes(mark) && blockElement && blockElement.markDefs && blockElement.markDefs.find(def => def._key === mark)).filter(Boolean);
