...
function validateManifest(manifest, plugin) {
  ...
  (manifest.parts || []).forEach((part, i) => {
    ...
    if (isImplementation && !hasPart) {
      throw new Error([baseError, 'A part that has a defined `path` needs to also define either `name` or `implements`', "See ".concat((0, _generateHelpUrl.generateHelpUrl)('plugin-parts-syntax'))].join('\n'));
    }

    if (!isDefined(part.path) && !isDefined(part.description)) {
      throw new Error([baseError, 'A part that has not defined a `path` needs to include a `description`', "See ".concat((0, _generateHelpUrl.generateHelpUrl)('plugin-parts-syntax'))].join('\n'));
    }
...
function validatePartName(name, baseError) {
  var examples = ['Examples:', '- part:package-name/part-name', '- part:package-name/part-name-style', '', "See ".concat((0, _generateHelpUrl.generateHelpUrl)('part-name-format'))].join('\n');
