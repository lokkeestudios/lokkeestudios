...
function mergeResult(plugins) {
  ...
  var partPlugins = plugins.map(plugin => {
    ...
    if (!Array.isArray(plugin.manifest.parts)) {
      var help = "See ".concat((0, _generateHelpUrl.generateHelpUrl)('plugin-parts-syntax'));
...
function assignPartImplementation(plugin, part, implementations, definitions, options) {
  ...
  if (prevDefinition && !prevDefinition.isAbstract) {
    ...
    throw new Error("".concat("Plugin ".concat(_current, " tried to implement part \"").concat(partName, "\", which is already declared") + " as a non-overridable part by ".concat(existing, " - ") + 'See ').concat((0, _generateHelpUrl.generateHelpUrl)('implement-non-overridable-part')));
