...
var AssetThumb = props => {
  ...
  var handleConfirmDelete = (0, _react.useCallback)(() => {
    setShowDeleteDialog(true);
  }, [setShowDeleteDialog]);
  var handleDialogClose = (0, _react.useCallback)(() => {
    setShowUsageDialog(false);
    setShowDeleteDialog(false);
  }, [setShowUsageDialog, setShowDeleteDialog]);
  var handleToggleUsageDialog = (0, _react.useCallback)(() => {
    setShowUsageDialog(true);
  }, [setShowUsageDialog]);
  var handleDeleteError = (0, _react.useCallback)(error => {
    toast.push({
      closable: true,
      status: 'error',
  ...
  var handleDeleteSuccess = (0, _react.useCallback)(() => {
    toast.push({
      status: 'success',
      title: 'Image was deleted'
    });
  }, [toast]);
  var handleDeleteAsset = (0, _react.useCallback)(() => {
    setIsDeleting(true);
  ...
  var handleMenuAction = (0, _react.useCallback)(action => {
  ...
  var originalFilename = asset.originalFilename,
      _id = asset._id,
      url = asset.url;
  var imgH = 200 * Math.max(1, DPI); // Mead can't convert gifs, so we might end up with large gifs that will cause the source window to use a lot of CPU
  // We instead force them to display as jpgs

  var imageUrl = url.includes('.gif') ? "".concat(url, "?h=").concat(imgH, "&fit=max&fm=jpg") : "".concat(url, "?h=").concat(imgH, "&fit=max");
...
var _default = /*#__PURE__*/_react.default.memo(AssetThumb);
