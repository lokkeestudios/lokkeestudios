...
function search(client, textTerm, type, options) {
  var searchWeighted = (0, _internal.createWeightedSearch)(type.to.map(crossDatasetType => ({
  ...
  (0, _operators.map)(results => results.map(result => result.hit)), (0, _operators.map)(_internal.collate), (0, _operators.map)(collated => collated.map(entry => ({
    id: entry.id,
    type: entry.type,
